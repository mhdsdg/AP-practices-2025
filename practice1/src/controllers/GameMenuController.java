...
public class GameMenuController {
    ...
    public Result showInformation(String countryString) {
        ...
        Countries country = switch (countryString) {
            case "German Reich" -> App.getCurrentGame().getGermany();
            case "Soviet Union" -> App.getCurrentGame().getSoviet();
            case "United Kingdom" -> App.getCurrentGame().getUk();
            case "United States" -> App.getCurrentGame().getUsa();
            default -> App.getCurrentGame().getJapan();
        };
        StringBuilder puppets = new StringBuilder();
        StringBuilder factions = new StringBuilder();
        for (Puppet puppet : country.puppets) {
            puppets.append(puppet.country.name).append(",");
        }
        for(Faction faction : country.factions){
            factions.append(faction.name.toLowerCase()).append(",");
        }
        if(!puppets.isEmpty()){
            puppets = new StringBuilder(puppets.substring(0, puppets.length() - 1));
        }
        if(!factions.isEmpty()){
            factions = new StringBuilder(factions.substring(0, factions.length() - 1));
        }
        System.out.println(country.name + "\nleader : " + country.leader.name + "\nstability : " +
                country.stability + "\nman power : " + country.manPower + "\nfuel : " + country.fuel +
                "\nsulfur : " + country.sulfur + "\nsteel : " + country.steel + "\nfaction : " + factions +
                "\npuppet : " + puppets);
        return new Result(true, puppets.toString());
    }
    public void tileOwner(String indexString) {
        if(isRegexWrong(indexString))return;
        int index = Integer.parseInt(indexString);
        if(isOutOfBounds(index, "tile doesn't exist"))return;
    ...
    public void tileNeighbours(String indexString) {
        ...
        for(Tile tile : App.getCurrentGame().getTiles().get(index - 1).groundNeighbours){
            indices.append(tile.index).append(" , ");
        }
        if(!indices.isEmpty()){
            indices = new StringBuilder(indices.substring(0, indices.length() - 3));
        }
    ...
    public void tileSetTerrain(String indexString , String terrainString) {
        if(isRegexWrong(indexString))return;
        int index = Integer.parseInt(indexString);
        if(isOutOfBounds(index, "you don't own this tile"))return;
        Game game = App.getCurrentGame();
        Tile tile = game.getTiles().get(index - 1);
    ...
    public void addBattalion(String indexString , String batType , String name) {
        if(isRegexWrong(indexString))return;
        int index = Integer.parseInt(indexString);
        if(isOutOfBounds(index, "tile is unavailable"))return;
        Game game = App.getCurrentGame();
        Tile tile = game.getTiles().get(index - 1);
        ...
        if(isBudgetLowBattalion(owner, bat)){
            System.out.println("daddy USA plz help us");
            return;
        }
        if(tileIsFull(batType, tile)) {
    ...
    public void moveBattalion(String indexString , String batName , String destString) {
        if(isRegexWrong(indexString) || isRegexWrong(destString))return;
        int index = Integer.parseInt(indexString);
        int dest = Integer.parseInt(destString);
        if(isOutOfBounds(index, "tile is unavailable") || isOutOfBounds(index, "tile is unavailable"))return;
        Game game = App.getCurrentGame();
        Tile tile = game.getTiles().get(index - 1);
        Tile destTile = game.getTiles().get(dest - 1);
        if(noAccess(game, tile) || noAccess(game, destTile)) {
            System.out.println("tile is unavailable");
            return;
        }
        ...
        if(tileIsFull(bat.type, destTile)) {
    ...
    public void upgradeBattalion(String indexString , String batName) {
        if(isRegexWrong(indexString))return;
        int index = Integer.parseInt(indexString);
        if(isOutOfBounds(index, "can't upgrade battalions on this tile"))return;
        Game game = App.getCurrentGame();
        Tile tile = game.getTiles().get(index - 1);
        ...
        if(isBudgetLowBattalion(country, bat)) {
    ...
    public void buildFactory(String indexString , String type , String name) {
        if(isRegexWrong(indexString))return;
        int index = Integer.parseInt(indexString);
        if(isOutOfBounds(index, "invalid tile"))return;
        Game game = App.getCurrentGame();
        Tile tile = game.getTiles().get(index - 1);
        if(noAccess(game, tile)) {
        ...
        Factory factory = switch (type) {
            case "steel factory" -> new SteelFactory(name);
            case "sulfur factory" -> new SulfurFactory(name);
            case "fuel refinery" -> new FuelRefinery(name);
            default -> null;
        };
        if(factory == null) {
    ...
    public void runFactory(String indexString , String name , String amountString){
        if(isRegexWrong(indexString) || isRegexWrong(amountString))return;
        int index = Integer.parseInt(indexString);
    ...
    public void attack(String indexString , String destString , String typeString){
        ...
        int index = Integer.parseInt(indexString);
        int dest = Integer.parseInt(destString);
        if(isOutOfBounds(index, "attacker tile unavailable"))return;
        Game game = App.getCurrentGame();
        Tile tile = game.getTiles().get(index - 1);
        ...
        if(isOutOfBounds(dest, "enemy tile unavailable for attacking"))return;
        Tile destTile = game.getTiles().get(dest - 1);
    ...
    public void puppet(String countryName){
        ...
        Game game = App.getCurrentGame();
        country = switch (countryName) {
            case "German Reich" -> game.getGermany();
            case "Soviet Union" -> game.getSoviet();
            case "United States" -> game.getUsa();
            case "United Kingdom" -> game.getUk();
            case "Japan" -> game.getJapan();
            default -> null;
        };
        if(country == null){
        ...
        for (Faction faction : game.getCurrentCountry().factions) {
            for (Countries count : faction.countries) {
                if (count.name.equals(countryName)) {
                    coFaction = true;
                    break;
                }
            }
    ...
    public boolean noAccess(Game game , Tile tile) {
        boolean hasAccess = tile.getOwner() == game.getPlayers().get(game.getCurrentPlayer());
        for (Puppet puppet : game.getCurrentCountry().puppets) {
            if (tile.getOwner() == puppet.country) {
                hasAccess = true;
                break;
            }
        }
    ...
    public boolean tileIsFull(String batType , Tile tile) {
        int batCount = 0;
    ...
    public boolean isBudgetLowBattalion(Countries country , Battalion bat) {
        return country.manPower < bat.cost.manPower ||
    ...
    public boolean isEnoughForFactory(Countries country , Factory factory , Tile tile) {
        ...
        return country.manPower >= manPowerCost &&
    ...
    public boolean isRegexWrong(String indexString ) {
    ...
    public boolean isOutOfBounds(int x , String message){
        if(x < 1 || x > 56) {
