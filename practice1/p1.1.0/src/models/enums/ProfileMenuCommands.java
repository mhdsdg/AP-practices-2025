...
public enum ProfileMenuCommands {
    ...
    ChangeCurrency("\\s*change-currency\\s+-n\\s+(?<newcurrency>.+?)\\s*"),
    ChangeUsername("\\s*change-username\\s+-n\\s+(?<newusername>.+?)\\s*"),
    ChangePassword("\\s*change-password\\s+-o\\s+(?<oldpassword>.+?)\\s+-n\\s+(?<newpassword>.+?)\\s*"),
    Back("\\s*back\\s*"),
    Exit("\\s*exit\\s*");
