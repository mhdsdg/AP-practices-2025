...
public class DashboardController {
    public Result createGroup(String name , String type) {
        ...
        GroupType groupType = switch (type) {
            case "Home" -> GroupType.Home;
            case "Zan-o-Bache" -> GroupType.Zan_o_Bache;
            case "Trip" -> GroupType.Trip;
    ...
    public void showOwnGroups() {
        ...
        if(user.groups.isEmpty()) System.out.println("");
    }
    ...
    public Result addExpense(String groupIDString , String typeOfSplit , String totalAmountString ,
                             String numberOfUsersString , HashMap<String, Integer> userValue ,
                             ArrayList<String> usernames , boolean sharesAreCorrect) {
        int groupID = Integer.parseInt(groupIDString);
        Group group = getGroupByID(groupID);
        ...
        for (String username : usernames) {
            ...
            if (user == null || !isUserInGroup(user , group)) {
                ...
                sum += userValue.get(username);
                users.put(user, (int) (userValue.get(username)*App.getLoggedInUser().getCurrency().value));
            }
        }
        if (!allUsersInGroup) {
        ...
        if (!totalAmountString.matches("\\d+") || !sharesAreCorrect) {
            return new Result(false, "expense format is invalid!");
        }
        int total = Integer.parseInt(totalAmountString);
        int totalAmount = (int)(total * App.getLoggedInUser().getCurrency().value);
        if (!(sum == total) && (typeOfSplit.equals("unequally"))) {
    ...
    public Result showBalance(String username) {
        ...
        for (Group group2 : groups) {
            groupNames.append(group2.getName()).append(", ");
        }
        if (groupNames.length() > 2) groupNames = new StringBuilder(groupNames.substring(0, groupNames.length() - 2));
        groupNames.append("!");
        if(totalBalance < 0) {
    ...
    public Result settleUpWith(String username , String amountString){
        User user1 = App.getLoggedInUser();
        ArrayList<Group> groups = new ArrayList<>(user1.groups);
        User user2 = getByUsername(username);
        if(user1.equals(user2)){
            return new Result(false, "you are settled with " + username + " now!");
        }
        if (user2 == null) {
            return new Result(false, "user not found!");
        }
        ...
        user1.balance.putIfAbsent(user2 , new HashMap<>());
        user2.balance.putIfAbsent(user1 , new HashMap<>());
    ...
    public Group getGroupByID(int id) {
        if (id > App.Groups.size() || id < 1) {return null;}
